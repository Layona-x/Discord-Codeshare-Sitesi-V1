...
app.get('/admin-codes/:userId',(req,res)=>{
  ...
  Code.find({userId:userId}).sort().then((codeResult)=>{
...
app.get('/code-edit/:codeId',(req,res)=>{
  ...
  let guild = client.guilds.cache.get(config.guildID)
  let member = guild.members.cache.get(userId)
  if(!member) return res.send("Sunucumuza Katılmasın Veya Siteye Giriş Yapmalisin")
...
app.get("/code/:codeId", async (req, res) => {
  ...
  if (!member) return res.send("Bu sayfaya girmek için sunucumuza katılmalısın!");
...
app.post("/code-add/:userId",upload.single('uploaded_file'),async (req, res) => {
  let userId = req.cookies.userId
  const guild = client.guilds.cache.get(config.guildID);
  const member = guild.members.cache.get(userId);
  const point = await User.findOne({ userId:userId })
  if (!member)
    return res.json({ 138: "Lütfen Giriş Yapınız!" });
  if(!member.hasPermission(8)) return res.json({ "404":"8 Perm Bulunmuyor Kod Yetkiniz Yok"})
...
app.post("/code-delete/:codeId", (req, res) => {
  ...
  let guild = client.guilds.cache.get(config.guildID)
  let member = guild.members.cache.get(userId)
  if(!member) return res.send('Sunucumuzda Bulunmuyorsun Veya Siteye Giriş yapmadın ')
...
app.post("/code-edit/:codeId", (req, res) => {
  ...
  let guild = client.guilds.cache.get(config.guildID)
  let member = guild.members.cache.get(userId)
  if(!member) return res.send("Sunucuya girmelisin veya siteye giriş yapmalısın")
  if(!member.hasPermission(8)) return res.send("Admin yetkin yok")
  const codeOwner = Code.findOne({ codeId: codeId });
  if (userId == codeOwner.codeOwner) {
    Code.findOneAndUpdate({ codeId:codeId }, {
      codeTitle: req.body.codeTitle,
      shortDescription: req.body.shortDescription,
      codePhoto: req.file.filename,
      codeText: req.body.codeText,
...
app.get('/ekip',(req,res)=>{
  ...
  res.render(`${__dirname}/src/pages/ekip.ejs`,{developer_1_name:developer_1_name,developer_1_avatar:developer_1_avatar,developer_2_name:developer_2_name,developer_2_avatar:developer_2_avatar})
})
...
app.get('/uptime/:userId',(req,res)=>{
  ...
  let guild = client.guilds.cache.get(config.guildID)
  let member = guild.members.cache.get(user)
  if(!member) return res.send("Lütfen Sunucumjza Girin veya Siteye Giriş Yapın")
...
app.get('/uptime-panel/:userId',(req,res)=>{
   ...
   let member = guild.members.cache.get(user)
    ...
    if(!member.hasPermission(8)) return res.send('Bu Siteye Erişmek İçin 8 Perme Sahip Olmalısın')
   Uptime.find().sort().then((uptimeResult)=>{
